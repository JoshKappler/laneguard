...
pnpm test         # 62 tests: parity, determinism, attacker behavior, ROC math
...
| attacker | verdict | how it's caught (or not) |
|---|---|---|
| naive scripted | **BOT 100%** | kinematics (jitter ≈ 0) + event provenance, in seconds |
| replay farm (injected) | **BOT 100%** | replay similarity — repeats in shape *and* timing |
| evasive generative | BOT ~17% | organic noise beats motor forensics; texture catches it slowly |
| **stealth camouflage** | **HUMAN** | **beats the entire client-side detector** |
...
contested space and genuinely crashes for it, and fakes aborted gestures). Keeping
that honest is the point — see the writeup.

## Why the economy wins

$5 entry, 20% rake → break-even win rate **62.5%**. Against a skill-matched
population (mean 50.0%, sd 3.8 pp over 300 games), the instant a bot clears the
rake wall it is already a **>3σ** outlier, and every extra dollar of profit pushes
it further out. There is no profitable-and-hidden zone. Rank by win-rate z-score,
corroborate with the behavioral signals, and false-positive bans stay near zero —
which matters when a ban withholds someone's cash.
