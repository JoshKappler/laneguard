...
/* First visit (no saved config, no ?c= permalink): boot the route planner
   bot (the strongest attacker, and the only one that turns a per-run
   profit) and pre-run 3 sim-minutes so every panel arrives populated. */
const DEMO_PRESET_ID = "route-planner";
  ...
  private demoBoot = false;
    ...
    if (this.demoBoot) {
      ...
      this.controller.annotate(
        "DEMO: pre-ran 3 minutes of the route planner bot so every panel arrives populated. Pick any preset on setup to run your own session."
      );
